...
var artistEditor,
    ...
    artistEditor : {
                   ...
                   destroyGeneric      : function ($element) {
                                             if ($element.length > 0) { // Not required, but 1 to 2 ms faster when this function is not actually needed.
...
// (window.console) ? console.timeEnd("makeTogglable") : '';
    ...
    makeTogglableTableCells : function () {
// (window.console) ? console.time("makeTogglableTableCells") : '';
        $cache.$releaseTable.bind("click", function (e) {
            var $clickTarget = $(e.target);
            if (!$clickTarget.hasClass('.editable')) { // If the click was on a child within the td, rather than on some exposed section of the cell itself,
                $clickTarget = $clickTarget.parents('.editable'); // get the "editable" td parent.
            }
            if ($clickTarget.length !== 0) { // The click occurred on a view mode cell.  (This check is not required, but it makes things faster when moving between exposed edit mode tds).
                artistEditor.destroyLookup();
                $clickTarget = $clickTarget.css("display", "none") // Hide it.
                                           .next('.hidden') // Get the closest sibling editing cell.
                                           .css("display", "table-cell"); // Show it.
                setTimeout(function () { // Delay this so we're outside of this click event before we trigger (and stop) another click event.
                    $clickTarget.find('input:visible:first, textarea:visible:first') // Find the first edit field,
...
// (window.console) ? console.timeEnd("makeTogglableTableCells") : '';
    },
...
if (window.console) {
   ...
   /* Set click behaviour for editable fields (where there is qty 1 of that field type). */
    ...
    /* Make editable tracklist table elements toggleable (from view to edit mode). */
    MusicBrainz.makeTogglableTableCells();
